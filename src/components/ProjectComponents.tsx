...
export function SafeImage({ src, alt, className }: { src: string; alt: string; className?: string }) {
    ...
    return (
        <div className={`relative flex items-center justify-center overflow-hidden bg-muted/10 ${className}`}>
            {isLoading && (
            ...
            {hasError && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted/10">
                    ...
                    <span className="text-[10px] font-sans font-bold uppercase tracking-widest text-muted-foreground/30 mt-4">Visual Pending</span>
                </div>
            )}
        </div>
    );
    ...
    return (
        ...
        >
            <div className="relative aspect-[16/11] overflow-hidden border border-border/10">
                <SafeImage src={project.thumbnail} alt={project.title} className="w-full h-full" />
                ...
                <button
                    ...
                    className={`absolute top-6 right-6 w-10 h-10 border border-border/20 flex items-center justify-center transition-all duration-500 hover:border-foreground ${isFavorite ? 'bg-primary border-primary text-background' : 'bg-background/60 text-foreground/40 hover:text-foreground'
                        }`}
                >
            ...
            <div className="mt-8 flex justify-between items-start gap-8">
                <div className="flex-1">
                    <div className="flex items-center gap-4 mb-4">
                        <span className="font-serif text-muted-foreground/30 text-lg italic">0{index + 1}</span>
                        <Separator className="w-12 bg-border/20" />
                        <span className="font-sans text-[9px] font-bold uppercase tracking-[0.2em] text-primary">{project.year}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-6">
                        <button className="flex items-center gap-3 font-sans text-[10px] font-bold uppercase tracking-[0.2em] transition-all group-hover:gap-6">
